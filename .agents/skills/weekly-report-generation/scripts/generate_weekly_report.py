...
def collect_appendix_sources(data_root: Path, week_start: str, week_end: str) -> dict[str, list[Path]]:
    ...
    snapshot_end_date = week_end_date + timedelta(days=1)
    ...
    def snapshot_files_within_week(subdir: str, prefix: str) -> list[Path]:
        ...
        for file in sorted(path.glob(f"{prefix}*.json")):
            ...
            try:
                ...
                continue
            if week_start_date <= snapshot_date <= snapshot_end_date:
                matched.append(file)
    ...
    for item in payload.get("instances", []):
        ...
        rows.append(
            [
                ...
                format_bytes(metric_average(metrics.get("used_memory_bytes"))),
...
def main() -> None:
    ...
    if input_path is None and args.week_start and args.week_end:
        default_input_path = Path("data/weekly") / f"weekly-input.{args.week_start}.{args.week_end}.json"
