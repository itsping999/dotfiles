...
- 附录资源明细优先从 `<data_root>/ecs`、`<data_root>/rds`、`<data_root>/redis`、`<data_root>/mongodb`、`<data_root>/slb`、`<data_root>/cdn`、`<data_root>/eip`、`<data_root>/shared-traffic-package` 自动生成，并补充组件级建议与改进措施。CDN 附录会交叉引用共享流量包中的 DCDN 数据。
...
2. 生成周报前，更适合先确认本周期依赖数据已经准备完成。
   ...
   - 组件指标：`<data_root>/ecs`、`<data_root>/rds`、`<data_root>/redis`、`<data_root>/mongodb`、`<data_root>/slb`、`<data_root>/cdn`、`<data_root>/eip`、`<data_root>/shared-traffic-package`
...
- 生成脚本必须把 ECS/RDS/Redis/MongoDB/K8S/SLB/CDN/EIP/共享流量包/证书/SMS/Voice/Email 13 类阿里云来源全部纳入采集完整性检查；若存在 `partial`、`incomplete`、`failed` 或缺文件，周报正文必须输出“阿里云采集完整性”小节并在风险项中标明阻塞服务，不能把阶段性产物描述为正式完成。
...
- **禁止上传含"未填写"的周报**：周报生成后、上传钉钉文档前，必须用 `grep -n "未填写" reports/weekly/<year>/<year>-W<week>.md` 检查。若存在任何"未填写"项，必须先补齐数据或在 weekly-input 中显式赋值，再重新生成周报，确认无"未填写"后才允许上传。
...
**每次调用 `generate_weekly_report.py` 重新生成周报时，以下运营指标会被重置为「未填写」：**
- 有效告警数
- 发布次数
- 发布成功率
- 回滚次数
...
**原因**：这些字段来自 `weekly-input` JSON 或手动填充，脚本无法从阿里云数据中自动获取全部字段（如有效告警数需人工判定），因此每次重新生成时会清空。

**恢复方法**：生成后立即用 `sed` 命令恢复：
```bash
sed -i '' \
  -e 's/有效告警数 | 未填写/有效告警数 | <N>/' \
  -e 's/发布次数 | 未填写/发布次数 | <N>/' \
  -e 's/发布成功率 | 未填写/发布成功率 | <N>%/' \
  -e 's/回滚次数 | 未填写/回滚次数 | <N>/' \
  -e 's/变更失败率（CFR） | 未填写/变更失败率（CFR） | <N>%/' \
  -e 's/发布 未填写 次，成功率 未填写，回滚 未填写 次/发布 <N> 次，成功率 <N>%，回滚 <N> 次/' \
  reports/weekly/<year>/<year>-W<week>.md
```

**数据来源**：
- 有效告警数：由运维团队提供（人工判定有效/无效）
- 发布次数/回滚次数：从阿里云 K8S 控制台「运维管理 → 任务」中统计
- 发布成功率/CFR：由发布次数和回滚次数计算得出

**建议**：将运营指标写入 `data/weekly/weekly-input.<start>.<end>.json`，脚本可自动读取，避免每次手动 sed。

## 11. 附录格式规范（W23 验证）

以下格式规则经 W23（2026-06-06 ~ 2026-06-12）实战验证，保证附录输出高度一致。

### 11.1 ECS 附录：单表合并

ECS 附录使用**单张 14 列表格**，合并 CPU/Memory/Disk/Connections 与 Network/DiskIO：

| 实例名 | 地域 | 状态 | CPU | 内存 | 磁盘 | 连接数 | 公网入 | 公网出 | 公网使用率 | 磁盘读 | 磁盘写 | IOPS读 | IOPS写 |

- 不具备网络或磁盘 IO 数据的实例，对应列填 `/`
- 不拆分为两张独立表格

### 11.2 MongoDB 附录：包含 CPU/内存/磁盘

MongoDB 附录表格除基本实例信息外，必须包含：
- CPU 使用率
- 内存使用率
- 磁盘使用率
- 连接数、QPS 等可用指标

若监控数据不可得（如控制台只显示最近 1h/24h），在对应列填 `/` 并在「重点观察」中说明。

### 11.3 SLB 附录：families 数组解析

SLB JSON 中 `families` 字段是**数组**（非对象），解析时需遍历数组并按 `family` 字段映射：
- `CLB` → 传统型负载均衡
- `ALB` → 应用型负载均衡
- `NLB` → 网络型负载均衡
- `GWLB` → 网关型负载均衡
...
# 错误做法（会丢失数据）
# get_nested(payload, "families", "classic_slb")
```

### 11.4 CDN 附录：DCDN 资源包交叉引用

CDN 附录除域名统计外，应交叉引用共享流量包中的 DCDN 资源包数据：
1. 从 `data/aliyun/shared-traffic-package/` 加载当周 JSON
2. 提取 `dcdn_resource_package_logs` 中的 DCDN 包信息
3. 按地域映射（如 `亚太1` ↔ `亚太1区`）关联到 CDN 域名
4. 展示：包总量、剩余量、剩余占比、包数量

### 11.5 共享流量包附录：简化为包列表

共享流量包附录使用**单张表格**展示资源包清单：

| 资源包类型 | 名称 | 实例 ID | 总量 | 剩余量 | 有效期至 |

- 只展示剩余的流量包及剩余量
- 不需要扣减明细表（deduction logs）
- 不需要汇总统计表（summary table）
- 包含所有类型的资源包：共享流量包、NAT 网关包、DCDN 流量包等

### 11.6 采集完整性章节（已移除）

W23 验证后确认：§5.9「阿里云采集完整性」章节与 §2.3 风险章节中的阻塞说明重复。
- 脚本中 `render_aliyun_collection_quality` 函数保留但不再在附录中调用
- 数据完整性问题统一在 §2.3「风险与关注项」中体现
- 若未来需要恢复，可重新在 `render_appendix_sections` 中添加调用
