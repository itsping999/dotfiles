...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="按前缀排除下载/更新/文件类接口，可多次传入；默认使用内置 4.1 排除清单",
    )
    parser.add_argument(
        "--exclude-static-prefix",
        action="append",
        default=None,
        help="按前缀排除静态资源请求，可多次传入；默认使用内置静态资源排除清单",
    )
...
def is_success_status_for_latency(status: int) -> bool:
...
def main() -> None:
    ...
    download_prefixes = args.exclude_download_prefix or DEFAULT_DOWNLOAD_PREFIXES
    static_prefixes = args.exclude_static_prefix or DEFAULT_STATIC_PREFIXES
    ...
    with log_path.open("r", encoding="utf-8", errors="replace") as handle:
        for raw_line in handle:
            ...
            if is_failed_status(parsed.status):
                failed_requests += 1
                continue
            if not is_success_status_for_latency(parsed.status):
                continue
